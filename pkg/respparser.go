...
func readSimpleString(idx *int, str *string) (string, error) {
	...
	if lim == -1 {
		return "", fmt.Errorf("%s","error of terminator wasn't found")
	}
	lim = lim + (*idx)
	out = (*str)[*idx:lim]
	*idx = lim + 1
...
func readErrorMSG(idx *int, str *string) (error, error) {
	...
	lim := strings.Index((*str)[*idx:], crlfTerminator)
	if lim == -1 {
		return nil, fmt.Errorf("%s","error of terminator wasn't found")
	}
	lim = lim + (*idx)
	out = (*str)[*idx:lim]
	*idx = lim + 1
	return fmt.Errorf("%s",out), nil
}
...
func readBoolean(idx *int, str *string) (bool, error) {
	if (*idx) >= len(*str) {
		return false, fmt.Errorf("%s","error of string lenght isn't enough")
	}
	if (*str)[(*idx)] == 't' {
		*idx += 2
		return true, nil
...
func readArray(idx *int, str *string) ([]interface{}, error) {
	...
	for i := 0; i < int(length); i++ {
		...
		default:
			return nil, fmt.Errorf("%s","error of unsupported datatype")
		}
		(*idx)++
	}
	return out, nil
...
func readBulkError(idx *int, str *string) (error, error) {
	...
	lim := (*idx) + int(length)
	if lim > len(*str) {
		return nil, fmt.Errorf("%s","error of string lenght isn't enough")
	}
	out = (*str)[*idx:lim]
	if out == "-1" {
		return nil, nil
